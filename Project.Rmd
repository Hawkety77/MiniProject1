...
```{r fig1, echo = FALSE, fig.height = 1.5, fig.width = 6.5, fig.cap="Example Histograms of Original Variables"}
...
```{r fig2, echo = FALSE, message = FALSE, fig.width = 6.5, fig.height = 1.5, fig.cap="X1 Example of Yeo-Johnson Transformation"}
library(MASS)
library(car)
...
qq_plot_with_ci <- function(x, conf = 0.95, n_highlight = 500, name = "") {
  n <- length(x)
  x_sorted <- sort(x)
  ...
  # Plot
  plot(q_theoretical, q_sample, main = paste0("X1 ", name),
...
par(mfrow=c(1,4))
hist(df$X1, main="X1 Original", xlab="X1", col="lightblue", border="black", breaks = 200)
qq_plot_with_ci(df$X1, n_highlight = 500, name = "Original")
hist(df_transformed$X1, main="X1 Transformed", xlab="X1", col="lightblue", border="black", breaks = 200)
qq_plot_with_ci(df_transformed$X1, n_highlight = 500, name = "Transformed")
```
...
Each of the other variables exhibited similar improvements in normality after the Yeo-Johnson transformation, but none achieved perfect normality. The heavy-tailed nature of the distributions persists to some extent across all variables.
...
```{r fig3, echo = FALSE, fig.height = 3, fig.cap="Original vs Yeo-Johnson Transformed QQ-Plots"}
...
is_fraud <- as.integer(seq_len(nrow(df)) %in% top_250_indices)
